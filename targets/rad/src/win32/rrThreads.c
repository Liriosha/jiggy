...
// 2003\Vc7\PlatformSDK\Include\MMSystem.h(2007) : warning C4201: nonstandard
...
typedef BOOL WINAPI
TryEnterCriticalSectionFunc(LPCRITICAL_SECTION lpCriticalSection);

typedef DWORD WINAPI SetThreadIdealProcessorFunc(HANDLE hThread,
...
typedef DWORD WINAPI SignalObjectAndWaitFunc(HANDLE hObjectToSignal,
...
static void RADLINK rr_set_thread_name(DWORD thread_id,
  ...
  if ((UINTa)idp <= 1) {
    if (idp == 0) {
      ...
      if (kernel32Handle != 0) {
        idp = (IsDebuggerPresentFunc *)GetProcAddress(kernel32Handle,
                                                      "IsDebuggerPresent");
        if (idp)
          goto found_idp;
      }
...
static DWORD WINAPI win32_shim(LPVOID param) {
  ...
  // set the thread name in the debugger (more reliable to do this on the thread
  // itself)
  if ((t->name) && (*t->name)) {
#ifndef __RADXENON__
...
// you can use the extra data functions to save info that would otherwise have
// to be tls-ed
...
RADDEFFUNC S32 RADLINK rrThreadGetExtraData(rrThread *t, S32 index,
                                            void **ret) {
  if (index >= RR_MAX_EXTRA_THREAD_DATA)
    return 0;
...
// number) returns 0 for any thread that isn't an rrthread
...
RADDEFFUNC U32 RADLINK rrThreadGetNumThreads(void) { return active_rrthreads; }

RADDEFFUNC rrbool RADLINK rrThreadCreate(rrThread *rr,
                                         thread_function *function,
                                         U32 stack_size, void *user_data,
                                         S32 flags, char const *name) {
  rriThread *thread;
  int index;
  int i;
  U64 count;
...
#ifdef _DEBUG
  typedef char test[(sizeof(rrThread) >= (sizeof(rriThread) + 4 + 15))
                        ? 1
                        : -1]; // make sure our rrthread isn't too big
#endif
...
// try to find a zero bit in this mask
      ...
      if (rrAtomicCmpXchg32(&thread_masks[i], mask | (1 << bit_index), mask) ==
          mask) {
...
got_slot:
  ...
  thread->status = 1;
  thread->done = 0;
  thread->handle = CreateThread(0, stack_size, win32_shim, rr, CREATE_SUSPENDED,
                                &thread->id); // jkr: assert non-zero stacksize?
  ...
  if (thread->handle == 0) {
    ...
    for (;;) {
      ...
      if (rrAtomicCmpXchg32(&thread_masks[index >> 5], mask & ~bit, mask) ==
          mask)
        break;
    }
...
/*
  ...
  resolution does not improve the accuracy of the high-resolution performance
  counter.
*/
#if defined(__RADXENON__) || defined(__RADXBOX__) || defined(__RADSEKRIT__)
    ;
...
// get the thread on a reasonable cpu
  ...
  if ((UINTa)stip <= 1) {
    if (stip == 0) {
      HANDLE kernel32Handle = GetModuleHandle(_T("KERNEL32"));
      if (kernel32Handle != 0) {
        stip = (SetThreadIdealProcessorFunc *)GetProcAddress(
            kernel32Handle, "SetThreadIdealProcessor");
        if (stip)
          goto found;
      }
...
RADDEFFUNC rrbool RADLINK rrThreadWaitDone(rrThread *rr, U32 ms, U32 *ecp) {
  ...
  if ((ms != RR_WAIT_INFINITE) &&
      ((thread->flags & rrThreadNeedWaitDoneTimeOuts) == 0))
    RR_BREAK();
#endif
...
static void thread_cleanup(rriThread *thread) {
  ...
  if (rrAtomicCmpXchgPointer((UINTa *)&thread->handle,
                             (UINTa)INVALID_HANDLE_VALUE, h) == h) {
    U32 old;
    ...
    for (;;) {
      ...
      if (rrAtomicCmpXchg32(&thread_masks[thread->index >> 5], mask & ~bit,
                            mask) == mask)
        break;
    }
...
RADDEFFUNC void RADLINK rrThreadCleanUp(rrThread *rr) {
  ...
  // if the thread that we are cleaning up is running, don't clear anything out,
  // just set it to by the thread on exit
...
RADDEFFUNC rrOSThreadType RADLINK rrThreadGetOSThread(rrThread *thread) {
  HANDLE h = 0;
  if (!rrThreadGetPlatformHandle(thread, &h))
    return 0;
  return (rrOSThreadType)(UINTa)h;
}
...
RADDEFFUNC rrOSThreadIDType RADLINK rrThreadGetOSThreadID(rrThread *thread) {
  DWORD id = 0;
  if (!rrThreadGetPlatformId(thread, &id))
    return 0;
...
RADDEFFUNC rrbool RADLINK rrMutexCreate(rrMutex *rr, S32 mtype) {
...
#ifdef _DEBUG
  typedef char test[(sizeof(rrMutex) >= (sizeof(rriMutex) + 4 + 15))
                        ? 1
                        : -1]; // make sure our rrmutex isn't too big
#endif
  ...
  RR_ASSERT((((U64)(UINTa)rr->i) + sizeof(*(rr->i))) <=
            (((U64)(UINTa)rr) + sizeof(rr->data)));
  ...
  if ((UINTa)tecs <= 1) {
    if (tecs == 0) {
      HANDLE kernel32Handle = GetModuleHandle(_T("KERNEL32"));
      if (kernel32Handle != 0) {
        tecs = (TryEnterCriticalSectionFunc *)GetProcAddress(
            kernel32Handle, "TryEnterCriticalSection");
        if (tecs)
          goto have_tecs;
      }
    ...
    // we don't have tryenter, so if we need timeouts, then we have to use a
    // mutex
...
have_tecs:
  ...
  // spincount is way better on real multicore systems ; has no affect on single
  // core
  if ((UINTa)icsasc <= 1) {
    if (icsasc == 0) {
      HANDLE kernel32Handle = GetModuleHandle(_T("KERNEL32"));
      if (kernel32Handle != 0) {
        icsasc = (InitializeCriticalSectionAndSpinCountFunc *)GetProcAddress(
            kernel32Handle, "InitializeCriticalSectionAndSpinCount");
        if (icsasc)
          goto found;
      }
...
RADDEFFUNC void RADLINK rrMutexDestroy(rrMutex *rr) {
  ...
  // tries to be sort of thread safe, but usually frees the rrMutex so not
  // really
  //  also really if you can destroy mutex from multiple threads you likely have
  //  horrible problems
...
#ifdef _DEBUG
  if (((p->flags & FLAGS_TYPE_MASK) == (int)rrMutexDontNeedTimeouts) &&
      (ms != RR_WAIT_INFINITE))
    RR_BREAK(); // you tried to use a non-infinite timeout, when you said you
                // didn't need them!

  if (((p->flags & FLAGS_TYPE_MASK) == (int)rrMutexNeedOnlyZeroTimeouts) &&
      (ms != 0) && (ms != RR_WAIT_INFINITE))
    RR_BREAK(); // you tried to use a non-zero timeout, when you said you only
                // need zero tests
#endif

  if (p->flags & FLAGS_TYPE_MUTEX) {
    switch (
        WaitForSingleObject(p->w.m, (ms == RR_WAIT_INFINITE) ? INFINITE : ms)) {
    case WAIT_OBJECT_0:
    case WAIT_ABANDONED:
      return 1;
...
RADDEFFUNC void RADLINK rrThreadYieldNotLower(void) { Sleep(0); }
...
RADDEFFUNC void RADLINK rrThreadYieldToAny(void) {
  ...
  if ((UINTa)stt <= 1) {
    if (stt == 0) {
      HANDLE kernel32Handle = GetModuleHandle(_T("KERNEL32"));
      if (kernel32Handle != 0) {
        stt = (SwitchToThreadFunc *)GetProcAddress(kernel32Handle,
                                                   "SwitchToThread");
        if (stt)
          goto found;
      }
...
// hyper-threads
//	on a non-hyper threaded system it is a NOP so it should always be called
// in spins
RADDEFFUNC void RADLINK rrThreadSpinHyperYield(void) {
// on hypeSpinBackOffrthreaded procs need this :
//	benign on non-HT
//	(actually better than benign - it improves performance even non-HT
// multi-core)
...
RADDEFFUNC rrbool RADLINK rrEventCreate(rrEvent *rr) {
  ...
  typedef char test[(sizeof(rrEvent) >= (sizeof(rriEvent) + 4))
                        ? 1
                        : -1]; // make sure our rrmutex isn't too big
#endif
  ...
  // RR_ASSERT( ( ((U64)rr->i) + sizeof(rriEvent) ) <= ( ((U64)rr) +
  // sizeof(rr->data) ) );
...
RADDEFFUNC rrbool RADLINK rrEventSignalAndWait(rrEvent *signalHandle,
                                               rrEvent *waitHandle,
                                               U32 millis) {
  RR_COMPILER_ASSERT(RR_WAIT_INFINITE == INFINITE);
...
#pragma warning(disable                                                        \
                : 4055) // 'type cast' : from data pointer 'void *' to function
                        // pointer 'SignalObjectAndWaitFunc (__stdcall *)'

  SignalObjectAndWaitFunc *pf =
      (SignalObjectAndWaitFunc *)rrAtomicLoadAcquirePointer(&soaw);
  if ((UINTa)pf <= 1) {
    if (pf == 0) {
      HANDLE kernel32Handle = GetModuleHandle(_T("KERNEL32"));
      if (kernel32Handle != 0) {
        pf = (SignalObjectAndWaitFunc *)GetProcAddress(kernel32Handle,
                                                       "SignalObjectAndWait");
      }
      if (pf == 0)
  ...
  hr = SignalObjectAndWait(signalHandle->i->handle, waitHandle->i->handle,
                           millis, FALSE);
...
RADDEFFUNC rrbool RADLINK rrSemaphoreCreate(rrSemaphore *rr, S32 initialCount,
                                            S32 maxCount) {
  rriSemaphore *sem;

#ifdef _DEBUG
  typedef char test[(sizeof(rrSemaphore) >= (sizeof(rriSemaphore) + 4 + 15))
                        ? 1
                        : -1]; // make sure our rrSemaphore isn't too big
#endif
...
#ifdef __RADSEKRIT__
  rr->i->hSemaphore = CreateSemaphoreExW(0, initialCount, maxCount, NULL, 0,
...
RADDEFFUNC rrbool RADLINK rrSemaphoreDecrementOrWait(rrSemaphore *rr, U32 ms) {
  ...
  dwWaitResult = WaitForSingleObject(rr->i->hSemaphore,
                                     (ms == RR_WAIT_INFINITE) ? INFINITE : ms);
...
U32 RADLINK tf(void *user_data) {
  printf("thread %d %d %X %s\n", rrThreadIndex(0), (U32)rrThreadUnique(0),
         rrThreadCurrent(), rrThreadName(0));
  Sleep(100);
  return 67;
}

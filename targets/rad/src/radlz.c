...
//   noise don't interfere with the compression of the other bits), and the
//   middle bits can be predicted from the top bits (whew)
...
typedef struct LZCDATA {
  ...
  U32 max_bytes, uniq_bytes, max_offset, max_offs, max_offsL, max_offsM,
      max_offsT;
  QUICKFIND **quick2;
  QUICKFIND **quick3;
  QUICKFIND **quick4;
...
static U32 long_lengths[4] = {MAX_LENS * 2, MAX_LENS * 3, MAX_LENS * 4,
                              MAX_LENS * 8};
...
static void scan_hashes(LZC lz, QUICKFIND *start, U32 which, U8 const *cur,
                        U32 cur_len, U32 stop_len, U32 *long_len,
                        void const **match, U32 *bit_pat, F32 *best_comp) {
  F32 compb;
  RADIPROB comp;
  ...
  do {
    ...
    //   if so, then we ain't going to find a closer match.  There are
    //   occassionally farther away matchs that compress better, but it's so
    //   rare that the speed savings this check gives us is worth it.
    if ((stop_len == *long_len) &&
        ((quick->addr + quick->run_count) <= (U8 *)*match))
      break;
    ...
    if (quick->addr[0] == cur[0]) {
      ...
      if (len >= *long_len) // gets very slightly better compression if this
                            // check is taken out, but it slows down
      {
        ...
        comp =
            Arith_inv_probability(lz->lens[lz->last_len], b + 1, MAX_LENS + 1);
        comp = Arith_combine_probabilities(
            comp, Arith_inv_probability(lz->offsl, GETL(off), lz->max_offsL));
        comp = Arith_combine_probabilities(
            comp, Arith_inv_probability(lz->offst, GETT(off), lz->offst_used));
        comp = Arith_combine_probabilities(
            comp, Arith_inv_probability(lz->offsm[GETT(off)], GETM(off),
                                        lz->offsm_used));
...
static U32 find_match(LZC l, U8 const *cur, U32 cur_len, F32 best_comp,
                      void const **match, U32 *bit_pat) {
  QUICKFIND *start;
  U32 best_len = 0;
  ...
  *match = 0;
  ...
  if (cur_len >= 3) {
    ...
    if (start) {
      scan_hashes(l, start, 2, cur, cur_len,
                  (cur_len <= long_lengths[3]) ? cur_len : long_lengths[3],
                  &best_len, match, bit_pat, &best_comp);
    }
  }
  ...
  if ((best_len <= 3) && (cur_len >= 3)) {
    ...
    if (start) {
      scan_hashes(l, start, 1, cur, cur_len, (cur_len <= 3) ? cur_len : 3,
                  &best_len, match, bit_pat, &best_comp);
    }
  }
  ...
  if ((best_len <= 2) && (cur_len >= 2)) {
    ...
    if (start) {
      scan_hashes(l, start, 0, cur, cur_len, (cur_len <= 2) ? cur_len : 2,
                  &best_len, match, bit_pat, &best_comp);
    }
  }
...
static void add_to_quick(LZC l, U8 const *cur, U32 cur_len) {
  U32 hash;
  ...
  //  do we need to set this position (we don't insert it, if the run is greater
  //  than 3)
  if (l->last_run <= 2) {
    buf->addr = cur;
...
static void get_max_offsets(U32 max_offset, U32 *low_max, U32 *med_max,
                            U32 *high_max) {
  *low_max = (max_offset >= LMAX) ? LMAX : (max_offset + 1);
  *med_max = ((max_offset >> LOW_BITS) >= MMAX)
                 ? MMAX
                 : ((max_offset >> LOW_BITS) + 1);
...
RADDEFFUNC U32 LZ_compress_alloc_size(U32 max_byte_value, U32 uniq_byte_values,
                                      U32 max_offset) {
  ...
  size += (high_max *
           Arith_compress_alloc_size(med_max - 1, med_max)); // for med offsets
...
RADDEFFUNC U32 LZ_compress_temp_size(U32 max_byte_value, U32 uniq_byte_values,
                                     U32 max_offset) {
...
RADDEFFUNC U32 LZ_compress_header_size(void) { return (sizeof(LZ_HEADER)); }

RADDEFFUNC LZC LZ_compress_open(void *ptr, void *compress_temp_buf,
                                U32 max_byte_value, U32 uniq_byte_values,
                                U32 max_offset) {
  U32 size, i;
  U8 *addr;
  LZC l = (LZC)ptr;
  ...
  for (i = 0; i <= ADDRESS_MASK; i++) {
    l->bytes[i] =
        Arith_open(addr, compress_temp_buf, max_byte_value, uniq_byte_values);
    addr += size;
  }
  ...
  for (i = 0; i <= MAX_LENS; i++) {
    l->lens[i] = Arith_open(addr, compress_temp_buf, MAX_LENS, MAX_LENS + 1);
  ...
  for (i = 0; i < l->max_offsT; i++) {
    l->offsm[i] =
        Arith_open(addr, compress_temp_buf, l->max_offsM - 1, l->max_offsM);
    addr += size;
  }

  l->offsl =
      Arith_open(addr, compress_temp_buf, l->max_offsL - 1, l->max_offsL);

  addr += Arith_compress_alloc_size(l->max_offsL - 1, l->max_offsL);

  l->offst =
      Arith_open(addr, compress_temp_buf, l->max_offsT - 1, l->max_offsT);

  return (l);
}

// Returns the header for this compression.
RADDEFFUNC void LZ_compress_get_header(LZC l, void *header) {
  U32 s1, s2;
  U32 j, i;
  ...
  for (j = 0; j < 4; j++) {
    ...
    for (i = 0; i < (MAX_LENS / 4); i++) {
      U32 u =
          Arith_compress_unique_values(l->lens[(j * (MAX_LENS / 4)) + i]) + 1;
      if (u > s1)
        s1 = u;
    }
    if (s1 > (MAX_LENS + 1))
...
RADDEFFUNC U32 LZ_compress(LZC l, ARITHBITS *ab, U8 const *input,
                           U32 input_len) {
  if (input_len < 1) {
    if (input_len == 0) {
      return (0);
    ...
    first_char_lit_size = Arith_combine_probabilities(
        lit_len_size,
        Arith_inv_probability(l->bytes[((UINTa)input) & ADDRESS_MASK], input[0],
                              l->max_bytes));
    ...
    if (input == l->save_pos) {
      ...
      len = find_match(l, input, input_len,
    ...
    if (len >= 2) {
      ...
      // if this is a short length, check against literal size and better
      // encoding
      if (len <= 5) {
        ...
        run_size = Arith_inv_probability(l->lens[l->last_len], bit_pat + 1,
                                         MAX_LENS + 1);

        run_size = Arith_combine_probabilities(
            run_size, Arith_inv_probability(l->offsl, GETL(off), l->max_offsL));
        run_size = Arith_combine_probabilities(
            run_size,
            Arith_inv_probability(l->offst, GETT(off), l->offst_used));
        run_size = Arith_combine_probabilities(
            run_size, Arith_inv_probability(l->offsm[GETT(off)], GETM(off),
                                            l->offsm_used));
        ...
        for (j = 1; j < len; j++) {
          ...
          Arith_adjust_probability(
              l->bytes[((UINTa)&input[j - 1]) & ADDRESS_MASK], input[j - 1], 1);

          lits_size = Arith_combine_probabilities(
              lits_size,
              Arith_combine_probabilities(
                  lit_len_size, Arith_inv_probability(
                                    l->bytes[((UINTa)&input[j]) & ADDRESS_MASK],
                                    input[j], l->max_bytes)));
        }
        ...
        for (t = 1; t < j; t++) {
          Arith_adjust_probability(
              l->bytes[((UINTa)&input[t - 1]) & ADDRESS_MASK], input[t - 1],
              -1);
        }
        ...
        // step 2
        // check whether we'd get a better match if we first encoded a literal
        // (lazy stab at flexible parsing)
        l->save_pos = input + 1;

        l->save_len = find_match(
            l, l->save_pos, input_len - 1,
            Arith_inv_probability_to_bits(Arith_combine_probabilities(
                lit_len_size, Arith_inv_probability(
                                  l->bytes[((UINTa)l->save_pos) & ADDRESS_MASK],
                                  l->save_pos[0], l->max_bytes))),
            &l->save_match, &l->save_bit_pat);

        {
          ...
          if (l->save_len >= 2) {
            bit1 = Arith_combine_probabilities(
                bit1, Arith_inv_probability(l->lens[0], l->save_bit_pat + 1,
                                            MAX_LENS + 1));
            // cast for 64-bit
            j = (U32)((l->save_pos) - (U8 *)l->save_match - 1);

            bit1 = Arith_combine_probabilities(
                bit1, Arith_inv_probability(l->offsl, GETL(off), l->max_offsL));
            bit1 = Arith_combine_probabilities(
                bit1,
                Arith_inv_probability(l->offst, GETT(off), l->offst_used));
            bit1 = Arith_combine_probabilities(
                bit1, Arith_inv_probability(l->offsm[GETT(off)], GETM(off),
                                            l->offsm_used));

            if ((((F32)(l->save_len + 1)) /
                 Arith_inv_probability_to_bits(bit1)) >
                (((F32)len) / Arith_inv_probability_to_bits(run_size))) {
              goto literal;
            }
          }
        }
      ...
      if (Arith_was_escaped(
              Arith_compress(l->lens[l->last_len], ab, bit_pat + 1))) {
      ...
      if (Arith_was_escaped(
              Arith_compress(l->offsm[GETT(off)], ab, GETM(off)))) {
        // dump the escaped value
    ...
    literal:
      ...
      // printf("%i: lit: %c %i\n",l->bytes_compressed, *input, ( (U32) input )
      // & ADDRESS_MASK );
      ++l->bytes_compressed;
      ...
      // dump the byte
      if (Arith_was_escaped(Arith_compress(
              l->bytes[((UINTa)input) & ADDRESS_MASK], ab, *input))) {
        // dump the escaped value
...
RADDEFFUNC U32 LZ_decompress_alloc_size(U32 max_byte_value,
                                        U32 uniq_byte_values, U32 max_offset) {
  U32 size, h, low_max, med_max, high_max;
  RR_UNUSED_VARIABLE(max_byte_value);
  ...
  size +=
      Arith_decompress_alloc_size(MAX_LENS + 1) * (MAX_LENS + 1); // for lengths
...
RADDEFFUNC LZD LZ_decompress_open(void *ptr, U32 max_byte_value,
                                  U32 uniq_byte_values, U32 max_offset) {
  U32 i, size;
  U8 *addr;
  LZD l = (LZD)ptr;
  ...
  for (i = 0; i <= ADDRESS_MASK; i++) {
    l->bytes[i] = Arith_open(addr, 0, max_byte_value, uniq_byte_values);
    addr += size;
  }
  ...
  for (i = 0; i <= MAX_LENS; i++) {
    l->lens[i] = Arith_open(addr, 0, MAX_LENS, MAX_LENS + 1);
  ...
  l->offst = Arith_open(addr + Arith_decompress_alloc_size(l->max_offsL), 0,
                        l->max_offsT - 1, l->max_offsT);
...
RADDEFFUNC U32 LZ_decompress_alloc_size_from_header(void *header) {
  ...
  size += Arith_decompress_alloc_size(GETT(h->uniq_offset_and_byte >> 9) +
                                      1); // for high offsets
  ...
  size += (Arith_decompress_alloc_size(h->uniq_lens & 255) *
           (MAX_LENS - ((MAX_LENS / 4) * 4) + 1));

  return (size);
}

RADDEFFUNC LZD LZ_decompress_open_from_header(void *ptr, void *header) {
  U32 i, j, size;
  U8 *addr;
  U32 uniq = 0;
  LZD l = (LZD)ptr;
  ...
  for (i = ((MAX_LENS / 4) * 4); i <= MAX_LENS; i++) {
    // This use of uniq should indeed inherit the value of the last iteration of
    // the previous loop
  ...
  l->offst =
      Arith_open(addr + Arith_decompress_alloc_size(l->max_offsL), 0,
                 l->max_offsT - 1, GETT(h->uniq_offset_and_byte >> 9) + 1);

  return (l);
}

RADDEFFUNC U32 LZ_decompress(LZD l, ARITHBITS *ab, U8 *output) {
  UINTa v;
  U32 escaped;
  ...
  if (v) {
    ...
    len =
        (v >= (MAX_LENS - 3)) ? long_lengths[v - (MAX_LENS - 3)] : (U32)(v + 1);
    ...
    *output = (U8)v;
    ...
    // printf("%i: lit: %c %i\n",l->bytes_decompressed, v, ( (U32) output ) &
    // ADDRESS_MASK);

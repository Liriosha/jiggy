...
static inline S32 __cntlzw(S32 arg) {
  U32 val;
  asm("cntlzw %0,%1" : "=r"(val) : "r"(arg));
  return val;
}

static inline float __frsqrte(float arg) {
  float val;
  asm("frsqrte %0,%1" : "=f"(val) : "f"(arg));
  return val;
}
...
inline static F64 radlog2(F64 X) { return (radlog(X) / LN2); }

#define rlwinm24023(d, s) __asm__("rlwinm %0,%1,24,0,23" : "=r"(d) : "r"(s))

#define rlwinm82431(d, s) __asm__("rlwinm %0,%1,8,24,31" : "=r"(d) : "r"(s))

#define rlwimi8815(d, s) __asm__("rlwimi %0,%1,8,8,15" : "=r"(d) : "r"(s))
...
static F64 inline radlog2(F64 X) { return (radlog(X) / LN2); }
...
static __inline U32 radloadu32(register U32 x) {
  return (__rlwimi(__rlwinm(x, 24, 0, 23), x, 8, 8, 15) |
          __rlwinm(x, 8, 24, 31));
}
...
#elif defined(__RADPSP2__) || defined(__RADIPHONE__) ||                        \
...
__inline static F64 radlog2(F64 X) { return (radlog(X) / LN2); }
...
static F64 __inline radlog2(F64 X) { return (radlog(X) / LN2); }
static U32 RADINLINE radloadu32(register U32 x) { return (_byteswap_ulong(x)); }
...
#if defined(_MSC_VER)
#pragma intrinsic(abs, log, fabs, sqrt, fmod, sin, cos, tan, asin, acos, atan, \
                  atan2, exp)
#endif
...
U32 mult64andshift(U32 mt1, U32 mt2, U32 shift);
#pragma aux mult64andshift =                                                   \
    "mul ebx"                                                                  \
    "shrd eax,edx,cl" parm[eax][ebx][ecx] modify[EDX eax];

S32 radabs(S32 ab);
#pragma aux radabs = "cdq"                                                     \
                     "xor eax,edx"                                             \
                     "sub eax,edx" parm[eax] modify[edx eax];
...
// #pragma intrinsic(abs, log, fabs, sqrt, fmod, sin, cos, tan, asin, acos,
// atan, atan2, exp)
...
#pragma aux mult64anddiv = "shl ebx,16"                                        \
                           "mov bx,ax"                                         \
                           "shrd eax,edx,16"                                   \
                           "mov ax,si"                                         \
                           "mul ebx"                                           \
                           ...
                           "shld edx,eax,16"                                   \
                           "and edx,0xffff"                                    \
                           "and eax,0xffff" parm[bx ax][dx si]                 \
    [cl] modify[ax bx cx dx si di] value[dx ax];

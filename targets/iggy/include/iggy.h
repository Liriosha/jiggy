...
#endif
    IggyTempRef
        temp_ref;  // An opaque temporary reference which you can
...
IDOCN typedef struct {
    ...
    rrbool(RADLINK* attach)(
        ...
        IggyForPerfmonFunctions*
            pmf);  // an Iggy file is trying to attach itself
...
typedef struct {
    ...
    F32 cx,
        cy;   // if IGGY_VERTEX_curve, control point on segment; ignored
              // otherwise
...
typedef enum IggyAntialiasing {
    ...
    IGGY_ANTIALIASING_PrettyGood =
        8,  // High-quality anti-aliasing on everything,

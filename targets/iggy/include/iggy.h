...
typedef enum IggyResult {
  ...
  IGGY_RESULT_Error_ProgramFlow =
      402, // an error occured with the user's program flow through the Iggy API
           // (e.g. reentrancy issues)
  ...
  IGGY_RESULT_Error_OutOfMemory =
      1001, // Iggy ran out of memory while processing the SWF. The Iggy player
            // is now invalid and you cannot do anything further with it (except
            // read AS3 variables). Should this happen, you'll want to
            // $IggyPlayerDestroy and reopen the $Iggy.
} IggyResult;
...
typedef enum IggyDatatype {
  ...
  IGGY_DATATYPE_array,  // Data of type Array in AS3 (appears in datatype query,
                        // never as arguments)
  IGGY_DATATYPE_object, // Data of type Object (or a subclass) in AS3 (appears
                        // in datatype query, never as arguments)
  IGGY_DATATYPE_displayobj, // Data of type DisplayObject (or a subclass) in AS3
                            // (only appears in callbacks)

  IGGY_DATATYPE_xml, // Data of type XML or XMLList in AS3 (appears in datatype
  ...
  IGGY_DATATYPE_namespace, // Data of type Namespace in AS3 (appears in datatype
                           // query, never as arguments)
  IGGY_DATATYPE_qname, // Data of type QName in AS3 (appears in datatype query,
                       // never as arguments)
  IGGY_DATATYPE_function, // Data of type Function in AS3 (appears in datatype
                          // query, never as arguments)
  IGGY_DATATYPE_class, // Data of type Class in AS3 (appears in datatype query,
                       // never as arguments)
} IggyDatatype;
...
typedef struct IggyStringUTF8 {
  ...
  S32 length; // Count of 8-bit bytes in <tt>string</tt>, not including the null
              // terminator
} IggyStringUTF8;
...
typedef struct IggyDataValue {
  ...
  union {
    ...
    IggyName fastname; // A fast name, valid if type =
                       // $(IggyDatatype::IGGY_DATATYPE_fastname); this is only
                       // an "in" type; Iggy will never define these itself
    void *userdata; // A userdata pointer from a DisplayObject, valid if type =
                    // $(IggyDatatype::IGGY_DATATYPE_displayobj)
    IggyValuePath
        *valuepath; // A path to an object in the AS3 VM, valid if type =
                    // $(IggyDatatype::IGGY_DATATYPE_valuepath); this is only an
                    // "in" type--Iggy will never output this
    IggyValueRef valueref; // An IggyValueRef, valid if type =
                           // $(IggyDatatype::IGGY_DATATYPE_valueref);  this is
...
typedef enum {
  ...
  IGGY_VERSIONED_BEHAVIOR_textfield_position =
      129, // This changes the behavior of textfield positioning as reported by
           // AS3 getBounds/getRect and width/height. Values with different
  ...
  IGGY_VERSIONED_BEHAVIOR_textfield_autoscroll =
      131, // This makes textfield autoscrolling behave specially: Valid values:
           // IGGY_VERSION_1_1_8, IGGY_VERSION_default
  IGGY_VERSIONED_BEHAVIOR_fast_text_effects =
      132, // This fixes the behavior of fast text effects to be in the correct
           // direction; Valid values: IGGY_VERSION_1_2_28, IGGY_VERSION_default
...
typedef enum IggyTelemetryAmount {
  IGGY_TELEMETRY_normal, // Normal amount for users debugging applications using
                         // Iggy
...
typedef struct {
  ...
  F32 width;  /* the objectspace width of the textfield at the time the text is
                 set */
  F32 height; /* the objectspace height of the textfield at the time the text is
                 set */
...
typedef struct {
  ...
  S32 greylist_size_in_bytes; // storage used to keep track of partially-garbage
                              // collected objects on the old heap
...
typedef struct IggyProperties {
  S32 movie_width_in_pixels; // the width of the "document" specified in the SWF
                             // file
  S32 movie_height_in_pixels; // the height of the "document" specified in the
                              // SWF file

  F32 movie_frame_rate_current_in_fps; // the current frame rate Iggy is trying
                                       // to achieve for the file
  F32 movie_frame_rate_from_file_in_fps; // the frame rate specified in the SWF
                                         // file
...
IDOCN typedef struct {
  ...
  S32(RADLINK *get_storage)(Iggy *swf, HIGGYEXP iggyexp,
                            U8 **buffer); // returns temporary storage Iggy can
                                          // use for assembling commands
  rrbool(RADLINK *attach)(
      Iggy *swf, HIGGYEXP iggyexp, iggyexp_detach_callback *cb, void *cbdata,
      IggyForPerfmonFunctions *pmf); // an Iggy file is trying to attach itself
                                     // to this connection (one at a time)
  rrbool(RADLINK *detach)(Iggy *swf,
                          HIGGYEXP iggyexp); // the current Iggy file should be
...
typedef struct {
  ...
  F32 descent;
  F32 line_gap;
  F32 average_glyph_width_for_tab_stops; // for embedded fonts, Iggy uses width
                                         // of 'g'
...
typedef struct {
  U8 *pixels_one_per_byte; // pixels from the top left, 0 is transparent and 255
                           // is opaque
  ...
  rrbool point_sample; // if true, the bitmap will be drawn with point sampling;
  ...
  F32 pixel_scale_correct; // the pixel_scale at which this character should be
                           // displayed at width_in_pixels
  F32 pixel_scale_min; // the smallest pixel_scale to allow using this character
                       // (scaled down)
  F32 pixel_scale_max; // the largest pixels cale to allow using this character
                       // (scaled up)
  void *user_context_for_free; // you can use this to store data to access on
                               // the corresponding free call
} IggyBitmapCharacter;
...
RADEXPFUNC void
    RADEXPLINK IggyFontInstallTruetypeUTF8(const void *truetype_storage,
                                           S32 ttc_index, const char *fontname,
...
typedef struct IggyCustomDrawCallbackRegion {
  ...
  F32 rgba_add[4]; // any additive color effect specified for the DisplayObject
                   // or its parents
...
typedef enum IggyAntialiasing {
  ...
  IGGY_ANTIALIASING_PrettyGood = 8, // High-quality anti-aliasing on everything,
                                    // but no rendertargets required
  IGGY_ANTIALIASING_Good =
      10, // High-quality anti-aliasing on everything (on platforms where GDraw
          // doesn't support rendertargets, such as the Wii, this behaves the
          // same as PrettyGood)
} IggyAntialiasing;
...
typedef struct {
  ...
  U32 base_old_amount;   // the base number of words to process on each minor
                         // cycle, default 200
  F32 old_heap_fraction; // the fraction 0..1 (default 0.125) of the outstanding
                         // allocations from the last major GC cycle to traverse
                         // during one GC cycle
  F32 new_allocation_multiplier; // a number from 1..infinity (default 2) which
                                 // is the amount of the allocations in the last
                                 // cycle to traverse
...
typedef struct {
  ...
  U32 increment_processing_bytes; // the number of bytes that were processed in
                                  // that event

  U32 last_slice_tenured_bytes; // the number of bytes that were tenured from
                                // young-to-old heap since the previous GC step
  U32 last_slice_old_allocation_bytes; // the number of bytes that were tenured
                                       // or were directly allocated from the
                                       // old heap since the previous GC step

  U32 heap_used_bytes; // the number of bytes in use in the old heap (the young
                       // heap is empty)
  ...
  U32 onstage_display_objects;  // the number of on-stage display objects
                                // (MovieClips, TextFields, Shapes, etc) visited
                                // during tenuring only
...
typedef enum IggyKeyloc {
  ...
  IGGY_KEYLOC_Right =
      2, // Specifies the right-hand-side key for keys with left/right variants
         // (such as $(IggyKeycode::IGGY_KEYCODE_SHIFT),
         // $(IggyKeycode::IGGY_KEYCODE_ALTERNATE), etc.) */

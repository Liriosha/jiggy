/*
   ...
   SORTINDEX - this the type name of an index to the data that you are sorting.
    ...
    SORTSWAP - this macro is called with two SORTINDEXes for two elements that
      ...
      should almost always use that will do the swap quickly for a specified
   type, so generally this macro will be defined something like:

        #define SORTSWAP( index1, index2 ) SORTSWAPDEFAULT( index1, index2,
   float )
      ...
      There is another predefined macro that you can use when you don't have the
   type of the data, but just a constant length.  If you wanted to use it, use
   something like:

        #define SORTSWAP( index1, index2 ) SORTSWAPLENGTH( index1, index2,
   sizeof( element ) + 4 )
     ...
     SORTSWAP3 - this macro is just like SORTSWAP, only it takes three elements
   to swap.  Again, we have the default macros, SORTSWAP3DEFAULT,
   SORTSWAP3LENGTH, and SORTSWAP3GENERIC that you can use to rapidly swap memory
   in a 3 ring. Note that if you write this macro yourself completely, the order
   of swapping is: ind3=ind2; ind2=ind1; ind1=original ind3;
     ...
     SORTKEYDECLARE *optional define* - sort.inl does it's compares by comparing
       ...
       Since we have the indexes that we are comparing to anyway, you don't
   *have* to define this macro at all - you can just compare the values in the
   two elements.  That is, you only need to declare a key if it will be faster
   than repeatedly dereferencing the elements for the comparisons.  Usually the
       smaller and the simpiler the comparisons, the more likely that declaring
       a key is worthwhile.

       So, let's say you have a structure and you have a name and a zip code
   that you are sorting by. To set this up, you would use something like this:
     ...
     SORTKEYSET *optional define* - this macro assigns a given indexed element's
       key values to the key variables that you defined with SORTKEYDECLARE.  If
   you didn't declare any key variables then you don't need to define this macro
       either. From the name/zip example above, you could use something like
   this:

         #define SORTKEYSET( index ) name_key = index->name_field;  zip_code =
   index->zip_field;



     SORTKEYISBEFORE, SORTKEYISAFTER, SORTKEYISEQUAL - these three macros do the
       actual comparison.  You must define all three because different
   comparisions are done at different places throughout the sort.  These macros
   are passed to SORTINDEX values - one that is the key index, and one that is
   the compare index.  If you used the SORTKEYDECLARE and SORTKEYSET macros,
   then you don't use the key index at all - you just use your saved key
   variables.  So, if you were sorting by an integer field, you could use macros
   like this:

         #define SORTKEYDECLARE() int key_value;
         #define SORTKEYSET( index ) key_value = index->int_field;
         #define SORTKEYISBEFORE( key_index, index ) key_value <
   index->int_field; #define SORTKEYISAFTER( key_index, index ) key_value >
   index->int_field; #define SORTKEYISEQUAL( key_index, index ) key_value ==
   index->int_field;



      SORTINDEXADD *optional define* - this macro returns a SORTINDEX advanced
   by an integer amount. You only need to define this macro in unusual
   situations (like when you are sorting a linked list or sorting on-disc data).
   By default, it is simply defined as:

          #define SORTINDEXADD( index, amt ) index + amt



      SORTINDEXNEXT *optional define* - this macro returns a SORTINDEX that has
   been advanced to the next element. You only need to define this macro in
   unusual situations. By default, it is simply defined as:
      ...
      SORTINDEXPREV *optional define* - this macro returns a SORTINDEX that has
   been backed up to the previous element. You only need to define this macro in
        unusual situations.  By default, it is simply defined as:
      ...
      SORTINDEXDIFF *optional define* - this macro returns the difference
   between two indexes as a count.  By default, it is simply defined as:

          #define SORTINDEXDIFF( index1, index2 ) index1 - index2



      SORTINDEXSMALLER *optional define* - this macro compares one *index* to
   another. You only need to define this macro in unusual situations. By
   default, it is simply defined as:
      ...
      SORTTINY *optional define* - this macro tells sort.inl to just use a tiny
        ...
        smaller but still fairly fast.  For simple "sort a list of strings"
   usage, normally sort.inl costs about 2K in code space.  But if you define
        SORTTINY, then we only use about 300 bytes.



      NDEBUG - you don't define this, your building make file or environment
   does - this is just the macro we assume is defined when building in release
   mode. We still work if it isn't defined, just not as quickly.
   ...
   Sort a list of integers:
     ...
     #define SORTSWAP3( ind1, ind2, ind3 ) SORTSWAP3DEFAULT( ind1, ind2, ind3,
   int )
   ...
   Sort two parallel arrays:
     ...
     #define SORTSWAP( ind1, ind2 ) SORTSWAPDEFAULT( names + ind1, names + ind2,
   char * ); \ SORTSWAPDEFAULT( zips + ind1, zips + ind2, int )
     #define SORTSWAP3( ind1, ind2, ind3 ) SORTSWAP3DEFAULT( names + ind1, names
   + ind2, names + ind3, char * ); \ SORTSWAP3DEFAULT( zips + ind1, zips + ind2,
   zips + ind3, int )

     #define SORTKEYDECLARE() char * key_name; int key_zip;
     #define SORTKEYSET( key_ind ) key_name = names[ key_ind ]; key_zip = zips[
   key_ind ];

     #define SORTKEYISBEFORE( key_ind, ind ) ( my_compare( key_name, names[ ind
   ], key_zip, zips[ ind ] ) < 0 ) #define SORTKEYISEQUAL( key_ind, ind ) (
   my_compare( key_name, names[ ind ], key_zip, zips[ ind ] ) == 0 ) #define
   SORTKEYISAFTER( key_ind, ind ) ( my_compare( key_name, names[ ind ], key_zip,
   zips[ ind ] ) > 0 )
   ...
   Generate a generic sorter that uses callbacks like CRT qsort (this is
     ...
     void generic_qsort( void * base, int count, int element_size, compare_func
   * comp )
     {
       #define SORTINDEX int
       #define SORTFIRSTINDEX 0
       ...
       #define SORTSWAP( ind1, ind2 ) SORTSWAPGENERIC( PTRADD( base, ind1 ),
   PTRADD( base, ind2 ), element_size ); #define SORTSWAP3( ind1, ind2, ind3 )
   SORTSWAP3GENERIC( PTRADD( base, ind1 ), PTRADD( base, ind2 ), PTRADD( base,
   ind3 ), element_size );

       #define SORTKEYISBEFORE( key_ind, ind ) ( comp( PTRADD( base, key_ind ),
   PTRADD( base, ind ) ) < 0 ) #define SORTKEYISEQUAL( key_ind, ind ) ( comp(
   PTRADD( base, key_ind ), PTRADD( base, ind ) ) == 0 ) #define SORTKEYISAFTER(
   key_ind, ind ) ( comp( PTRADD( base, key_ind ), PTRADD( base, ind ) ) > 0 )
   ...
   When we pick our median of three for the pivot value, we sort the values we
   looked at, as long as we are there.  This gives a 8% speed improvement and we
   don't have to handle groups of 3 specifically since it just happens for free
   (we do still special case groups of 2 and 4, though).

   Our swap helper macros do a lot of trickiness to get the compiler to give
   optimal register use during swaps for any element size up to 16 bytes (this
   gives us 80% improvments in places).  For elements larger than that we use
   dwords to swap which is way faster than bytes.

   The handling of equivalent elements is handled extra carefully to balance
   needless swaps and extra compares in later stages.  We also use a pivot
   "area", not just a single pivot value.
   ...
   detect this, however, not by comparing sizes, but by comparing our final
   pivot index to the halfway index that we used in the median of three - this
   is faster and I haven't seen it done before.
   ...
   less than 128th of the original length.  This is kind of cool because 128th
   of a partition that is less than 128 elements is zero, so the same check for
   seeing whether we have a bad split prevents heap sort from running on lists
   smaller than 128 elements (where even with quadratic-ish behavior, qsort is
   fine).
   ...
   you handle those cases, it was always a lose to special case other small
   sizes.
   ...
   Our heap sort code is the very smallest and more stripped that I've ever
   seen. It uses no sub-functions (can't, since we are declared inline), but
   also does no duplication of code.  It kind of interlaces the pre-heap setup
   with the code that actually sorts the heap values with some duff-ish
   weirdness.  It is goto and logic weirdness heavy.  But it's fast and very
   small.
...
#define SORTSWAPSPECIFIC(addr1, addr2, kind)                            \
    do {                                                                \
        ...
        ((kind * SORTRESTRICT)(void* SORTRESTRICT)addr1)[0] =           \
            ((kind * SORTRESTRICT)(void* SORTRESTRICT)addr2)[0];        \
...
#define SORTSWAP3SPECIFIC(addr1, addr2, addr3, kind)                    \
    do {                                                                \
        ...
        ((kind * SORTRESTRICT)(void* SORTRESTRICT)addr3)[0] =           \
            ((kind * SORTRESTRICT)(void* SORTRESTRICT)addr2)[0];        \
        ((kind * SORTRESTRICT)(void* SORTRESTRICT)addr2)[0] =           \
            ((kind * SORTRESTRICT)(void* SORTRESTRICT)addr1)[0];        \
...
#define SORTSWAPLENGTH(addr1, addr2, len) SORTSWAPGENERIC(addr1, addr2, len)
...

{
    ...
    (defined(PRAGMA_STRUCT_PACKPUSH) && PRAGMA_STRUCT_PACKPUSH)
#pragma pack(push, 1)
#else
#pragma pack(1)
#endif
#endif
    typedef struct SORTPACKED __ssl3 {
        short s;
        char c;
    } __ssl3;
    typedef struct SORTPACKED __ssl5 {
        int i;
        char c;
    } __ssl5;
    typedef struct SORTPACKED SORTALIGN(2) __ssl6 {
        int i;
        short s;
    } __ssl6;
    typedef struct SORTPACKED __ssl7 {
        int i;
        short s;
        char c;
    } __ssl7;
    typedef struct SORTPACKED SORTALIGN(8) __ssl8 {
        SORTU64 u;
    } __ssl8;
    typedef struct SORTPACKED __ssl9 {
        SORTU64 u;
        char c;
    } __ssl9;
    typedef struct SORTPACKED SORTALIGN(2) __ssl10 {
        SORTU64 u;
        short s;
    } __ssl10;
    typedef struct SORTPACKED __ssl11 {
        SORTU64 u;
        short s;
        char c;
    } __ssl11;
    typedef struct SORTPACKED SORTALIGN(4) __ssl12 {
        SORTU64 u;
        int i;
    } __ssl12;
    typedef struct SORTPACKED __ssl13 {
        SORTU64 u;
        int i;
        char c;
    } __ssl13;
    typedef struct SORTPACKED SORTALIGN(2) __ssl14 {
        SORTU64 u;
        int i;
        short s;
    } __ssl14;
    typedef struct SORTPACKED __ssl15 {
        SORTU64 u;
        int i;
        short s;
        char c;
    } __ssl15;
    typedef struct SORTPACKED SORTALIGN(16) __ssl16 {
        SORTU64 u1;
        SORTU64 u2;
    } __ssl16;
#if !defined(R5900)
...
#pragma pack()
    ...
    left = (SORTINDEX)(SORTFIRSTINDEX);
    right = (SORTINDEX)(SORTLASTINDEX);
    if (SORTINDEXSMALLER(right, SORTINDEXNEXT(SORTINDEXNEXT(left))))
        goto no_sort;
#ifdef SORTLENGTH
    length = SORTLENGTH;
#else
    ...
    for (;;) {
        while (SORTINDEXSMALLER(left, right)) {
            ...
            if (length == 2) {
                SORTKEYSET(right);
                if (SORTKEYISBEFORE(right, left)) SORTSWAP(left, right);
                break;
            } else {
                ...
                if (length == 4) {
                    ...
                    else if (SORTKEYISBEFORE(piv, left))
                        SORTSWAP(left, piv);
                    break;
                }

                {
                    ...
                    while (1) {
                        do {
                            r = SORTINDEXPREV(r);
                            if (!SORTINDEXSMALLER(l, r)) goto parted;
                ...
                } while ((SORTINDEXSMALLER(rpiv, right)) &&
                         (SORTKEYISEQUAL(piv, rpiv)));
                ...
                } while ((SORTINDEXSMALLER(left, lpiv)) &&
                         (SORTKEYISEQUAL(piv, lpiv)));

                // is the small side on the left or the right?
                if (SORTINDEXSMALLER(piv, mid)) {
                    // left partition is smaller

                    // stick the larger partition on the stack (invert ptr to
                    // signify heapsort)
                    if (mpl < (length >> 7)) {

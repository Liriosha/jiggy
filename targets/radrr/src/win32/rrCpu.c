...
static S32 readcpu() {
  ...
  if (!checked) {
    ...
    if (kernel32Handle != 0)
      glpi = (LPFN_GLPI)GetProcAddress(kernel32Handle,
    ...
    while (len >= sizeof(SYSTEM_LOGICAL_PROCESSOR_INFORMATION)) {
      ...
      case rRelationProcessorPackage:
        // this gives us a bitmask of cpus that share a physical package (a
        // socket)
        if ((U32)(p - pk) < maxcpus)
...
RADDEFFUNC S32 RADLINK rrGetSpinCount(void) {
  ...
  if (s_spinCount == -1) {
    ...
    // !! be careful ! rrGetTotalCPUs() might use rrThread primitives which call
    // me ! first store something other than -1 so I won't recurse :

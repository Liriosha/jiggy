...
#include <xtl.h>
    ...
    defined(__NT__) || defined(__WIN32__)
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
extern "C" void Platform_SeekFromBeginning(PLATFORM_FILE *i_File,
                                           S32 i_Offset) {
  SetFilePointer(*(HANDLE *)i_File->plat_specific, i_Offset + i_File->start_pos,
                 0, FILE_BEGIN);
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
extern "C" U32 Platform_ReadFile(PLATFORM_FILE *i_File, void *o_Buffer,
                                 U32 i_ReadBytes) {
  DWORD BytesRead = 0;
  ReadFile(*(HANDLE *)i_File->plat_specific, o_Buffer, i_ReadBytes, &BytesRead,
           0);
...
extern "C" int Platform_OpenFile(struct PLATFORM_FILE *i_File, char const *fn) {
  ...
  HANDLE FileHandle =
      CreateFile(fn, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING,

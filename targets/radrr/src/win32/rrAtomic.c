...
extern "C" long __cdecl _InterlockedCompareExchange(volatile long *p, long n,
                                                    long p_compare);
...
extern "C" __int64 __cdecl _InterlockedCompareExchange64(volatile __int64 *dest,
                                                         __int64 newv,
                                                         __int64 p_compare);
...
In 32 bit builds, 64-bit values on the stack are NOT aligned to 8 bytes by the
compiler automatically.  This can cause atomicity failures.

I try to have asserts about required alignment in rrAtomics, but it's something
to be aware of.

I believe that statics/globals are aligned properly, which is what people
normally use as atomic values
...
U64 rrAtomicLoadAcquire64(U64 volatile const *src) {
  ...
  // old way: (dirties the cache with a bogus write, so now we do a read with
  // x87)
...
U32 rrAtomicCmpXchg32(U32 volatile *pDestVal, U32 newVal, U32 compareVal) {
  ...
  return _InterlockedCompareExchange((LONG *)pDestVal, (LONG)newVal,
                                     (LONG)compareVal);
}

U64 rrAtomicCmpXchg64(U64 volatile *pDestVal, U64 newVal, U64 compareVal) {
  RR_ASSERT((((UINTa)pDestVal) & 7) == 0);
  return _InterlockedCompareExchange64((__int64 *)pDestVal, (__int64)newVal,
                                       (__int64)compareVal);
}

// CAS like C++0x
// atomically { if ( *pDestVal == *pOldVal ) { *pDestVal = newVal; } else {
// *pOldVal = *pDestVal; } }
...
rrbool rrAtomicCAS128(ATOMIC128 volatile *pDestVal, ATOMIC128 *pOldVal,
  ...
  unsigned char did = _InterlockedCompareExchange128(
      (__int64 *)&pDestVal, pNew64[0], pNew64[1], (__int64 *)&pOldVal);
  return (rrbool)did;
}

#else

extern "C" extern int __cdecl asm_x64_cmpxchg128(volatile void *pVal,
                                                 void *pOld, const void *pNew);

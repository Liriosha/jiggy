...
#define COUNTS_SIZE(value) \
    (sizeof(COUNTTYPE) * ((get_binary_table_size(value + 1) + 3) & ~3))
...
RADDEFFUNC ARITH Arith_open(void* ptr, void* compress_temp_buf, U32 max_value,
                            U32 unique_values) {
    ARITH a;
    ...
    if (a) {
        ...
        a->values = (COUNTTYPE*)((char*)a + (sizeof(ARITHDATA) +
                                             COUNTS_SIZE(unique_values)));
        ...
        if (a->rescale < 128)
            a->rescale = 128;
        else if (sizeof(COUNTTYPE) == 2)
            if (a->rescale > (65536 - 128)) a->rescale = (65536 - 128);
    }
...
static U32 tree_to_linear(COUNTTYPE* current, COUNTTYPE* end, U32 level,
                          U32 size) {
    U32 ret;
    ...
    if (level > 1) {
        ...
        if (current < end)
            current[0] = (COUNTTYPE)tree_to_linear(
                current + level, end, level >> 1, size - current[0]);
        else
            current[0] = 0;
    } else {
        if (level < 1) {
...
static U32 linear_to_tree(COUNTTYPE* current, COUNTTYPE* end, U32 level,
                          U32 count) {
    U32 ret;
    ...
    if (level > 1) {
        if (current < end)
            ...
            ret = 0;
        current[0] =
            (COUNTTYPE)linear_to_tree(current - level, end, level >> 1, count);
...
// These functions rescale the counts every a->rescale symbols.  Because they
// are
...
static void rescale_compress(ARITH a) {
    ...
    tree_to_linear(a->counts + a->tree_top, a->counts + a->unique_symb + 1,
                   (a->tree_top + 1) >> 1, a->tree_tot_count);
    ...
    // place the new greatest value into the final bin position (minimize total
    // increments)
    if ((max) && (a->tree_top > 1)) {
        j = a->tree_top - 1;
        if (j == 0) ++j;
    ...
    // if the tree now has unused symbols, then make sure the escape code has a
    // count
    ...
    a->tree_tot_count =
        linear_to_tree(a->counts + a->tree_top, a->counts + a->unique_symb + 1,
                       (a->tree_top + 1) >> 1, 0);
}
...
#undef check_level
    ...
    radassert((symb_range > 0) &&
              ((symb_offset + symb_range) <= a->tree_tot_count));
...
RADDEFFUNC U32 Arith_decompress(ARITH a, ARITHBITS* ab) {
    ...
    // take the offset and find the symbol, then get the exact offset and size
    // of the symbol
    {
        COUNTTYPE* current;
        S32 amt_remaining;
...
RADDEFFUNC RADIPROB Arith_inv_probability(ARITH a, U32 value, U32 scale) {
    ...
    new_symb:
        ret = ((F32)scale) *
              (((F32)a->tree_tot_count) / (F32)get_symb_range(a, 0));
    } else {
        U32 symb_range;

        symb_range = get_symb_range(a, pos);
        if (pos == 0) goto new_symb;
...
#ifdef TESTPROBUSAGE
RADDEFFUNC RADIPROB Arith_combine_probabilities(RADIPROB iprob1,
                                                RADIPROB iprob2) {
    F32 ret;

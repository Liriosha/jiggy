...
RADDEFFUNC ARITH Arith_open(void *ptr, void *compress_temp_buf, U32 max_value,
  ...
  if (a) {
    ...
    a->values = (COUNTTYPE *)((char *)a +
                              (sizeof(ARITHDATA) + COUNTS_SIZE(unique_values)));
...
static U32 tree_to_linear(COUNTTYPE *current, COUNTTYPE *end, U32 level,
  ...
  if (level > 1) {
    ...
    if (current < end)
      current[0] = (COUNTTYPE)tree_to_linear(current + level, end, level >> 1,
                                             size - current[0]);
    else
      current[0] = 0;
  } else {
...
RADDEFFUNC U32 Arith_decompress(ARITH a, ARITHBITS *ab) {
  ...
  // take the offset and find the symbol, then get the exact offset and size of
  // the symbol
  {
    COUNTTYPE *current;
    S32 amt_remaining;

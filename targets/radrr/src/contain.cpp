...
element * listAdd( list * l, char * name, int id_number ); // if we are handling
allocations element * listAdd( list * l, element * item );             // if we
aren't handling allocation
...
(*REQUIRED IF WE DO ALLOCATION*)  #define CONTAINER_ADD_FIELDS  S32 data, char *
key
...
(*REQUIRED IF WE DO ALLOCATION*)  #define CONTAINER_ADD_ASSIGN( item )  strcpy(
(item)->name, key); (item)->zip = data;
...
(*REQUIRED IF SORTED*)  #define CONTAINER_COMPARE_ITEMS( item1, item2 ) stricmp(
...
(*REQUIRED IF USING FIND*)  #define CONTAINER_COMPARE_FIND_FIELDS( item )
stricmp( key, (item)->name )
...
#define CONTAINER_DEBUG_FORMAT_STRING( buf, item, skew ) sprintf( buf, "%0.0f,%i
%c", (item)->key, (item)->data, skew );
...
#define CONTAINER_STRUCT_ALREADY_DEFINED 0
   ...
   If you define CONTAINER_STRUCT_ALREADY_DEFINED, then contain.inl no longer
generates a structure definition for the container itself.  Instead, it expects
...
// This can't go inside a push/pop since it is checked for at the end of the
// compilation unit.
...
#define CONTAINER_NEED_UNUSED                               \
    (CONTAINER_NEED_REMOVE || CONTAINER_NEED_REMOVEFIRST || \
     CONTAINER_NEED_REMOVELAST || CONTAINER_DO_ALLOCATION)
#endif
...
// The second clause detects NaCl, which is x64 with 32 bit pointers.
    ...
    (!defined(__SIZEOF_PTRDIFF_T__) || __SIZEOF_PTRDIFF_T__ != 4)
...
typedef char
    CONTAINER_INTPTR_SizeCheck[sizeof(CONTAINER_INTPTR) == sizeof(void*) ? 1
                                                                         : -1];
...
#define SET_PTR(overloaded, value) \
    (overloaded).bits = (((overloaded).bits & 3) | ((CONTAINER_INTPTR)(value)))
#define GET_PTR(overloaded) ((CONTAINER_ITEM_TYPE*)((overloaded).bits & ~3))

#define SET_LEFT_SKEW(item)                                                  \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_LEFT_NAME)      \
        ->bits =                                                             \
        (((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_LEFT_NAME) \
             ->bits &                                                        \
         ~3)
#define SET_NO_SKEW(item)                                                     \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_LEFT_NAME)       \
        ->bits =                                                              \
        ((((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_LEFT_NAME) \
              ->bits &                                                        \
...
#define SET_SKEW(item, value)                                                 \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_LEFT_NAME)       \
        ...
        ((((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_LEFT_NAME) \
          ...
          ~3) |                                                               \
         (value))
#define GET_SKEW(item)                                                   \
    (((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_LEFT_NAME) \
         ->bits &                                                        \
     3)

#define IS_LEFT_SKEWED(item)                                              \
    ((((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_LEFT_NAME) \
          ->bits &                                                        \
      3) < 1)
#define IS_RIGHT_SKEWED(item)                                             \
    ((((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_LEFT_NAME) \
          ->bits &                                                        \
      3) > 1)
#define IS_NOT_SKEWED(item)                                               \
    ((((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_LEFT_NAME) \
          ->bits &                                                        \
      3) == 1)

#define SET_LEFT(item, value)                                                 \
    SET_PTR(                                                                  \
        *((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_LEFT_NAME), \
        value)
#define GET_LEFT(item) \
    GET_PTR(           \
        *((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_LEFT_NAME))

#define SET_RIGHT(item, value)                                           \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_RIGHT_NAME) \
        ->ptr = (value)
#define GET_RIGHT(item) \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_RIGHT_NAME)->ptr
...
#define SET_LEFT(item, value)                                                  \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_LEFT_NAME)->ptr = \
        (value)
#define GET_LEFT(item) \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_LEFT_NAME)->ptr
...
#define SET_UNUSED_COUNT(item, value)             \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item) \
         ->CONTAINER_UNUSED_COUNT_NAME)           \
        ->bits = (value)
#define GET_UNUSED_COUNT(item)                    \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item) \
         ->CONTAINER_UNUSED_COUNT_NAME)           \
        ->bits
#define SET_UNUSED_NEXT(item, value)                                           \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_UNUSED_NEXT_NAME) \
        ->ptr = (value)
#define GET_UNUSED_NEXT(item)                                                  \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_UNUSED_NEXT_NAME) \
        ->ptr
#endif

#define SET_NEXT(item, value)                                                  \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_NEXT_NAME)->ptr = \
        (value)
#define GET_NEXT(item) \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_NEXT_NAME)->ptr
#define SET_PREV(item, value)                                                  \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_PREV_NAME)->ptr = \
        (value)
#define GET_PREV(item) \
    ((MakeNamePostAlways(OVERLOADED_PTR)*)&(item)->CONTAINER_PREV_NAME)->ptr
...
static void MakeNamePost(add_new_buffer)(CONTAINER_NAME* sl, int num_items) {
    ...
    b = (MakeNamePostAlways(ITEM_BUFFER)*)CONTAINER_MALLOC(
        sizeof(MakeNamePostAlways(ITEM_BUFFER)) +
        (num_items - 1) * sizeof(CONTAINER_ITEM_TYPE));
    if (b) {
...
static MakeNamePostAlways(ITEM_BUFFER) *
    MakeNamePost(free_buffer)(MakeNamePostAlways(ITEM_BUFFER) * b) {
...
static CONTAINER_ITEM_TYPE* MakeNamePost(get_unused_item)(CONTAINER_NAME* sl) {
    ...
    if (sl->CONTAINER_UNUSED_MEMBER == 0) {
        ...
        if (sl->CONTAINER_UNUSED_MEMBER == 0) return 0;
    }
    ...
    if (GET_UNUSED_COUNT(new_item) > 1) {
        ...
        SET_UNUSED_COUNT(sl->CONTAINER_UNUSED_MEMBER,
                         GET_UNUSED_COUNT(new_item) - 1);
        SET_UNUSED_NEXT(sl->CONTAINER_UNUSED_MEMBER, GET_UNUSED_NEXT(new_item));
    } else {
        sl->CONTAINER_UNUSED_MEMBER =
            GET_UNUSED_NEXT(sl->CONTAINER_UNUSED_MEMBER);
    }
...
static void MakeNamePost(add_to_unused)(CONTAINER_NAME* sl,
                                        CONTAINER_ITEM_TYPE* remove) {
#if CONTAINER_DO_ALLOCATION
...
static void MakeNamePost(rotate_left)(CONTAINER_ITEM_TYPE* item,
                                      MakeNamePostAlways(OVERLOADED_PTR) *
                                          node) {
...
static void MakeNamePost(rotate_right)(CONTAINER_ITEM_TYPE* item,
                                       MakeNamePostAlways(OVERLOADED_PTR) *
                                           node) {
    ...
    CONTAINER_NEED_REMOVELAST

// left side has shrunk so do the rebalance - returns 1 if need to rebalance
// higher up, 0 if done

static int MakeNamePost(left_has_shrunk)(MakeNamePostAlways(OVERLOADED_PTR) *
                                         node) {
    ...
    if (IS_LEFT_SKEWED(item)) {
        ...
        if (IS_RIGHT_SKEWED(right)) {
            ...
            MakeNamePost(rotate_right)(
                right, (MakeNamePostAlways(
                           OVERLOADED_PTR)*)&item->CONTAINER_RIGHT_NAME);
            MakeNamePost(rotate_left)(item, node);
            return (1);
        }
    } else {
...
// right side has shrunk so do the rebalance - returns 1 if need to rebalance
// higher up, 0 if done

static int MakeNamePost(right_has_shrunk)(MakeNamePostAlways(OVERLOADED_PTR) *
                                          node) {
    ...
    if (IS_RIGHT_SKEWED(item)) {
        ...
        if (IS_LEFT_SKEWED(left)) {
            ...
            MakeNamePost(rotate_left)(
                left, (MakeNamePostAlways(
                          OVERLOADED_PTR)*)&item->CONTAINER_LEFT_NAME);
            MakeNamePost(rotate_right)(item, node);
            return (1);
        }
    } else {
...
static void MakeNamePost(get_and_remove_top)(CONTAINER_NAME* sl) {
    ...
    MakeNamePostAlways(OVERLOADED_PTR) *
        stack[33];  // would store 8 billion items - probably safe
    int stack_ptr;
    ...
    // put the first right branch at the top of the stack (in case we're
    // deleting the root)
...
static void MakeNamePost(get_and_remove_bottom)(CONTAINER_NAME* sl) {
    ...
    MakeNamePostAlways(OVERLOADED_PTR) *
        stack[32];  // would store 4 billion items - probably safe
    int stack_ptr;
    MakeNamePostAlways(OVERLOADED_PTR) * node;
    ...
    // put the first left branch at the top of the stack (in case we're deleting
    // the root)
    ...
    // now walk back up the path we took rotating if necessary along the way to
    // rebalance
    while (stack_ptr > 1) {
        --stack_ptr;
...
static void MakeNamePost(get_and_remove)(CONTAINER_NAME* sl,
                                         CONTAINER_ITEM_TYPE* remove_item) {
    ...
    for (;;) {
        ...
        remove_left:
            ...
            if (if_left == 0) return;  // remove_item not found
        ...
        remove_right:
            ...
            if (if_right == 0) return;  // remove_item not found

            stack[stack_ptr] = node;
            side_stack[stack_ptr++] = 1;

            node =
                (MakeNamePostAlways(OVERLOADED_PTR)*)&i->CONTAINER_RIGHT_NAME;
            i = if_right;
        } else {
#if CONTAINER_NEED_EXACT_DUPE_REMOVAL
            if (remove_item < i)
...
// remove the node
    ...
    if (left == 0) {
        CONTAINER_ASSERT((right == 0) ||
                         ((GET_LEFT(right) == 0) && (GET_RIGHT(right) == 0)));
        ...
        for (;;) {
            ...
            node =
                (MakeNamePostAlways(OVERLOADED_PTR)*)&j->CONTAINER_RIGHT_NAME;
            j = r;
        }
        ...
        // patch the stack so that the deleted item is gone and the right most
        // remains
        stack[save_stack_ptr + 1] =
            (MakeNamePostAlways(OVERLOADED_PTR)*)&j->CONTAINER_LEFT_NAME;
    }
...
// balances left side - returns 1 when a higher rebalance may be necessary, 0
// when done

static int MakeNamePost(left_has_grown)(MakeNamePostAlways(OVERLOADED_PTR) *
                                        node) {
    CONTAINER_ITEM_TYPE *item, *left;
...
// balances right side - returns 1 when a higher rebalance may be necessary, 0
// when done

static int MakeNamePost(right_has_grown)(MakeNamePostAlways(OVERLOADED_PTR) *
                                         node) {
    CONTAINER_ITEM_TYPE *item, *right;
...
static void MakeNamePost(insert_into_tree)(CONTAINER_NAME* sl,
                                           CONTAINER_ITEM_TYPE* new_item) {
    MakeNamePostAlways(OVERLOADED_PTR) *
        stack[32];  // would store 2 billion items - probably safe (AVL trees
                    // can be unbalanced, so it isn't 4 billion)
    char side_stack[32];
    int stack_ptr;
    MakeNamePostAlways(OVERLOADED_PTR) * node;
    ...
    do {
        --stack_ptr;
        if (side_stack[stack_ptr] == 0) {
            if (!MakeNamePost(left_has_grown)(stack[stack_ptr])) break;
        } else {
            if (!MakeNamePost(right_has_grown)(stack[stack_ptr])) break;
        }
    } while (stack_ptr);
}
...
CONTAINER_FUNCTION_DECORATE(int)
MakeNamePost(Initialize)(CONTAINER_NAME* sl
#if CONTAINER_DO_ALLOCATION
                         ,
                         int HintItems)
#else
...
CONTAINER_FUNCTION_DECORATE(CONTAINER_ITEM_TYPE*)
MakeNamePost(Add)(CONTAINER_NAME* sl,
#if CONTAINER_DO_ALLOCATION
...
#if CONTAINER_EMIT_CODE
{
...
#if CONTAINER_KEEP_LINKED_LIST
    {
        ...
        CONTAINER_ASSERT((sl->CONTAINER_FIRST_MEMBER == i) &&
                         (GET_PREV(sl->CONTAINER_FIRST_MEMBER) == 0));
...
CONTAINER_FUNCTION_DECORATE(CONTAINER_ITEM_TYPE*)
MakeNamePost(Remove)(CONTAINER_NAME* sl, CONTAINER_ITEM_TYPE* item)
#if CONTAINER_EMIT_CODE
{
#if CONTAINER_SORTED
    if (sl->CONTAINER_FIRST_MEMBER == item) {
        ...
        if (sl->CONTAINER_ROOT_MEMBER) {
            ...
            CONTAINER_ASSERT((sl->CONTAINER_LAST_MEMBER == i) &&
                             (GET_NEXT(sl->CONTAINER_LAST_MEMBER) == 0));
        }
#endif
#endif
    } else {
...
#if CONTAINER_NEED_REMOVEFIRST && \
    (CONTAINER_SORTED || CONTAINER_KEEP_LINKED_LIST)

CONTAINER_FUNCTION_DECORATE(CONTAINER_ITEM_TYPE*)
MakeNamePost(RemoveFirst)(CONTAINER_NAME* sl)
#if CONTAINER_EMIT_CODE
{
    if (sl->CONTAINER_FIRST_MEMBER == 0) return (0);
...
#if CONTAINER_NEED_REMOVELAST && \
    (CONTAINER_SORTED || CONTAINER_KEEP_LINKED_LIST)

CONTAINER_FUNCTION_DECORATE(CONTAINER_ITEM_TYPE*)
MakeNamePost(RemoveLast)(CONTAINER_NAME* sl)
#if CONTAINER_EMIT_CODE
{
    if (sl->CONTAINER_LAST_MEMBER == 0) return (0);
...
CONTAINER_FUNCTION_DECORATE(CONTAINER_ITEM_TYPE*)
MakeNamePost(First)(CONTAINER_NAME* sl)
#if CONTAINER_EMIT_CODE
{
...
CONTAINER_FUNCTION_DECORATE(CONTAINER_ITEM_TYPE*)
MakeNamePost(Next)(CONTAINER_NAME* sl, CONTAINER_ITEM_TYPE* item)
#if CONTAINER_EMIT_CODE
{
#if CONTAINER_KEEP_LINKED_LIST
...
#else
    {
        ...
        if (i) {
            ...
            return ((sl->CONTAINER_LAST_MEMBER == item)
                        ? 0
                        : sl->CONTAINER_LAST_MEMBER);
        }
    }
#endif
}
...
CONTAINER_FUNCTION_DECORATE(CONTAINER_ITEM_TYPE*)
MakeNamePost(Previous)(CONTAINER_NAME* sl, CONTAINER_ITEM_TYPE* item)
#if CONTAINER_EMIT_CODE
{
#if CONTAINER_KEEP_LINKED_LIST
#if !CONTAINER_SORTED && CONTAINER_DO_ALLOCATION
...
#else
    {
        ...
        if (it) {
            ...
            // not found
            return ((sl->CONTAINER_FIRST_MEMBER == item)
                        ? 0
                        : sl->CONTAINER_FIRST_MEMBER);
        }
    }
#endif
}
...
CONTAINER_FUNCTION_DECORATE(CONTAINER_ITEM_TYPE*)
MakeNamePost(Last)(CONTAINER_NAME* sl)
#if CONTAINER_EMIT_CODE
{
...
#if (CONTAINER_NEED_FIND) && defined(CONTAINER_FIND_FIELDS) && \
    defined(CONTAINER_COMPARE_FIND_FIELDS) &&                  \
    (CONTAINER_SORTED || CONTAINER_KEEP_LINKED_LIST)

CONTAINER_FUNCTION_DECORATE(CONTAINER_ITEM_TYPE*)
MakeNamePost(Find)(CONTAINER_NAME* sl, CONTAINER_FIND_FIELDS)
#if CONTAINER_EMIT_CODE
{
#if CONTAINER_SORTED
    CONTAINER_ITEM_TYPE* i;
...
#if (CONTAINER_NEED_FINDGT) && defined(CONTAINER_FIND_FIELDS) && \
    defined(CONTAINER_COMPARE_FIND_FIELDS) &&                    \
    (CONTAINER_SORTED || CONTAINER_KEEP_LINKED_LIST)

CONTAINER_FUNCTION_DECORATE(CONTAINER_ITEM_TYPE*)
MakeNamePost(FindGT)(CONTAINER_NAME* sl, CONTAINER_FIND_FIELDS)
#if CONTAINER_EMIT_CODE
{
#if CONTAINER_SORTED
    ...
    while (i) {
        if (CONTAINER_COMPARE_FIND_FIELDS(i) < CONTAINER_COMPARE_EQUAL_VALUE) {
            if (best) {
                if (CONTAINER_COMPARE_ITEMS(i, best) <
                    CONTAINER_COMPARE_EQUAL_VALUE) {
                    best = i;
                }
            } else {
                best = i;
...
#if (CONTAINER_NEED_FINDLT) && defined(CONTAINER_FIND_FIELDS) && \
    defined(CONTAINER_COMPARE_FIND_FIELDS) &&                    \
    (CONTAINER_SORTED || CONTAINER_KEEP_LINKED_LIST)

CONTAINER_FUNCTION_DECORATE(CONTAINER_ITEM_TYPE*)
MakeNamePost(FindLT)(CONTAINER_NAME* sl, CONTAINER_FIND_FIELDS)
#if CONTAINER_EMIT_CODE
{
#if CONTAINER_SORTED
    CONTAINER_ITEM_TYPE *i, *last_right;
...
CONTAINER_FUNCTION_DECORATE(void)
MakeNamePost(Clear)(CONTAINER_NAME* sl)
#if CONTAINER_EMIT_CODE
{
#if CONTAINER_DO_ALLOCATION
...
CONTAINER_FUNCTION_DECORATE(void)
MakeNamePost(FreeMemory)(CONTAINER_NAME* sl)
#if CONTAINER_EMIT_CODE
{
    MakeNamePostAlways(ITEM_BUFFER) * b;

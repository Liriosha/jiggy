...
// Granny has it's own version of these to prevent link collisions.  Note that
// Granny doesn't define SetMemory, since if you need to hijack Granny's
// allocation, it goes through GrannySetAllocator.
#if (defined(BUILDING_GRANNY) && BUILDING_GRANNY) || \
    (defined(BUILDING_GRANNY_STATIC) && BUILDING_GRANNY_STATIC)
...
RADDEFFUNC void PTR4* RADLINK radmalloci(U32 numbytes, char const* info,
                                         U32 line);
#else

...
VARBITS can either be in a struct , or in local variables set up by
VARBITSLOCAL(name)
...
#define VarBitsOpen(vb, pointer)                        \
    {                                                   \
        (vb).init = pointer;                            \
        if (((U32)pointer) & 4) {                       \
            (vb).bits = *((U32 * RADRESTRICT) pointer); \
            (vb).cur = ((char*)pointer) + 4;            \
            (vb).bitlen = 32;                           \
        } else {                                        \
            (vb).cur = pointer;                         \
            (vb).bits = (vb).bitlen = 0;                \
...
#define VarBitsPut1(vb, boolean)                      \
    {                                                 \
        if (boolean) (vb).bits |= (1 << (vb).bitlen); \
        if ((++(vb).bitlen) == 32) {                  \
            *((U32*)(vb).cur) = (vb).bits;            \
            (vb).cur = ((char*)((vb).cur) + 4);       \
            (vb).bits = (vb).bitlen = 0;              \
...
#define VarBitsConvertPutToGet(gvb, pvb)                       \
    {                                                          \
        if (((pvb).bitlen)) {                                  \
            (gvb).bits = (*((U32*)(pvb).cur)) >> (pvb).bitlen; \
            (gvb).bitlen = BITSTYPELEN - (pvb).bitlen;         \
            (gvb).cur = ((char*)((pvb).cur) + 4);              \
        } else {                                               \
            (gvb).bits = 0;                                    \
            (gvb).bitlen = 0;                                  \
            (gvb).cur = (pvb).cur;                             \
...
#define VarBitsSize(vb)                                   \
    ((U32)((((char*)(vb).cur) - ((char*)(vb).init)) * 8 + \
           (vb).bitlen))  // in bits !!
...
// WARNING : this getbitlevel only works on *constant* U16 values ! (up to
// 65535)
...
#define VarBitsPos(vb) \
    ((U32)((((U8*)(vb).cur) - ((U8*)(vb).init)) * 8 - (vb).bitlen))
...
// for debugging: causes crash on zero: #define GetBitsLen(val)
// (((val)==0)?(((U8*)val)[0]=0):(0xffffffffL>>(U32)(32-(val))))

// get 1 bit
//    best to use like : if ( VarBitsGet1(vb,i) )
//
// This *doesn't* set i to the result - that's just a temp - the expression is
// the result!!!
...
#define VarBitsGet1(vb, i)                                        \
    (((vb).bitlen == 0)                                           \
         ...
         : (i = ((vb).bits), ((vb).bits) >>= 1, --((vb).bitlen)), \
     (i & 1))

// not USE64BITVB safe!
#define VarBitsGet1LE(vb, i)                                                   \
    (((vb).bitlen == 0)                                                        \
         ? (i = radloadu32ptr((vb).cur), ((vb).cur) = ((char*)((vb).cur)) + 4, \
            ((vb).bits) = ((U32)i) >> 1, ((vb).bitlen) = 31)                   \
         : (i = ((vb).bits), ((vb).bits) >>= 1, --((vb).bitlen)),              \
     (i & 1))
...
#define VarBitsGetWithCheck(v, typ, vb, len, endp, dowhat)         \
    {                                                              \
        if (((vb).bitlen) < (len)) {                               \
            ...
            if (((U8*)((vb).cur)) >= ((U8*)(endp)))                \
                dowhat nb = *((BITSTYPE * RADRESTRICT)((vb).cur)); \
            v = (typ)((((vb).bits) | (nb << ((vb).bitlen))) &      \
                      GetBitsLen(len));                            \
            ((vb).bits) = nb >> ((len) - ((vb).bitlen));           \
            ((vb).bitlen) = ((vb).bitlen) + BITSTYPELEN - (len);   \
...
#define VarBitsGetWithCheckBE(v, typ, vb, len, endp, dowhat)     \
    {                                                            \
        if (((vb).bitlen) < (len)) {                             \
            register BITSTYPE nb;                                \
            if (((U8*)((vb).cur)) >= ((U8*)(endp)))              \
                dowhat nb = radloadu32ptrBE((vb).cur);           \
            v = (typ)((((vb).bits) | (nb << ((vb).bitlen))) &    \
                      GetBitsLen(len));                          \
            ((vb).bits) = nb >> ((len) - ((vb).bitlen));         \
            ((vb).bitlen) = ((vb).bitlen) + BITSTYPELEN - (len); \
...
#define VarBitsGetWithCheckLE(v, typ, vb, len, endp, dowhat)     \
    {                                                            \
        if (((vb).bitlen) < (len)) {                             \
            register BITSTYPE nb;                                \
            if (((U8*)((vb).cur)) >= ((U8*)(endp)))              \
                dowhat nb = radloadu32ptr((vb).cur);             \
            v = (typ)((((vb).bits) | (nb << ((vb).bitlen))) &    \
                      GetBitsLen(len));                          \
            ((vb).bits) = nb >> ((len) - ((vb).bitlen));         \
            ((vb).bitlen) = ((vb).bitlen) + BITSTYPELEN - (len); \
...
#define VarBitsLocalGet(v, typ, vb, len)                                    \
    {                                                                       \
        ...
        if ((vb##bitlen) < len) {                                           \
            _bits |=                                                        \
                (((U64)(*((U32 * RADRESTRICT)(vb##cur)))) << (vb##bitlen)); \
...
#define VarBitsLocalGet1WithCheck(v, vb, endp, dowhat)                   \
    {                                                                    \
        if ((vb##bitlen) == 0) {                                         \
            if (((U8*)(vb##cur)) >= ((U8*)(endp)))                       \
                dowhat(vb##bits) = *((BITSTYPE * RADRESTRICT)(vb##cur)); \
            (vb##cur) = ((char*)(vb##cur)) + BITSTYPEBYTES;              \
...
#define VarBitsLocalGet1WithCheckBE(v, vb, endp, dowhat)     \
    {                                                        \
        if ((vb##bitlen) == 0) {                             \
            if (((U8*)(vb##cur)) >= ((U8*)(endp)))           \
                dowhat(vb##bits) = radloadu32ptrBE(vb##cur); \
            (vb##cur) = ((char*)(vb##cur)) + BITSTYPEBYTES;  \
            (vb##bitlen) = (BITSTYPELEN);                    \
        }                                                    \
        --(vb##bitlen);                                      \
...
#define VarBitsLocalGet1WithCheckLE(v, vb, endp, dowhat)    \
    {                                                       \
        if ((vb##bitlen) == 0) {                            \
            if (((U8*)(vb##cur)) >= ((U8*)(endp)))          \
                dowhat(vb##bits) = radloadu32ptr(vb##cur);  \
            (vb##cur) = ((char*)(vb##cur)) + BITSTYPEBYTES; \
            (vb##bitlen) = (BITSTYPELEN);                   \
        }                                                   \
        --(vb##bitlen);                                     \
...
#define VarBitsLocalGetIfxSM(v, typ, vb, len, i, x, mask)                     \
    (vb##bitlen == 0)                                                         \
        ? (i = *((BITSTYPE * RADRESTRICT)(vb##cur)),                          \
           (vb##cur) = ((char*)(vb##cur)) + BITSTYPEBYTES,                    \
           ((i & 1) == x) ? (v = (typ)((i >> 1) mask),                        \
                             (vb##bits) = ((BITSTYPE)i) >> (len + 1),         \
                             (vb##bitlen) = (BITSTYPELEN - 1 - len), x)       \
                          : ((vb##bits) = (((BITSTYPE)i) >> 1),               \
                             (vb##bitlen) = (BITSTYPELEN - 1), !x))           \
        : (--(vb##bitlen),                                                    \
           (((vb##bits) & 1) == x)                                            \
               ? (((vb##bitlen) < len)                                        \
                      ? (i = *((BITSTYPE * RADRESTRICT)(vb##cur)),            \
                         (vb##cur) = (((char*)(vb##cur)) + BITSTYPEBYTES),    \
                         v = (typ)(((vb##bits >> 1) | (i << vb##bitlen))      \
                                       mask),                                 \
                         (vb##bits) = (i >> ((len) - vb##bitlen)),            \
                         (vb##bitlen) = ((vb##bitlen) + BITSTYPELEN - (len)), \
                         x)                                                   \
                      : (v = (typ)(((vb##bits) >> 1) mask),                   \
                         (vb##bits) >>= (len + 1), (vb##bitlen) -= len, x))   \
               : ((vb##bits) >>= 1, !x))
...
#define VarBitsLocalGet1BE(vb, i)                                              \
    ((vb##bitlen == 0)                                                         \
         ? (i = radloadu32ptrBE(vb##cur),                                      \
            (vb##cur) = ((char*)(vb##cur)) + BITSTYPEBYTES,                    \
            (vb##bits) = ((BITSTYPE)i) >> 1, (vb##bitlen) = (BITSTYPELEN - 1)) \
         : (i = (vb##bits), (vb##bits) >>= 1, --(vb##bitlen)),                 \
     (i & 1))
...
#define VarBitsLocalGet1(vb, i)                                     \
    (((vb##bitlen == 0)                                             \
          ? ((vb##bits) = *((BITSTYPE * RADRESTRICT)(vb##cur)),     \
             (vb##cur) = ((char*)(vb##cur)) + 4, (vb##bitlen) = 32) \
          : 0),                                                     \
     (i = (vb##bits) & 1, (vb##bits) >>= 1, --(vb##bitlen)), (i))
...
#define VarBitsLocalPos(vb, origvb)                           \
    ((U32)((((char*)vb##cur) - ((char*)(origvb)->init)) * 8 + \
           (32 - vb##bitlen)))  // in bits !!
...
#define VarBitsBPutOpen(vb, ptr)             \
    {                                        \
        (vb).cur = (vb).init = (void*)(ptr); \
        (vb).bits = (vb).bitlen = 0;         \
        (vb).isPut = 1;                      \
    }
#define VarBitsBGetOpen(vb, ptr)             \
    {                                        \
        (vb).cur = (vb).init = (void*)(ptr); \
        (vb).bits = (vb).bitlen = 0;         \
        (vb).isPut = 0;                      \
...
#define VarBitsBPut(vb, val, size)                     \
    do {                                               \
        ...
        if ((vb).bitlen + __s >= BITSTYPELEN) {        \
            U32 __r = __s + (vb).bitlen - BITSTYPELEN; \
...
#define VarBitsBPuta1(vb)                       \
    {                                           \
        (vb).bits = (vb).bits + (vb).bits + 1;  \
...
#define VarBitsBPut1(vb, bit)                                \
    {                                                        \
        (vb).bits = (vb).bits + (vb).bits + ((bit) ? 1 : 0); \
        if ((++(vb).bitlen) == BITSTYPELEN) {                \
            *((U32*)(vb).cur) = (vb).bits;                   \
            (vb).cur = ((char*)((vb).cur) + 4);              \
            (vb).bits = (vb).bitlen = 0;                     \
...
#define VarBitsBPutAlign(vb)                \
    {                                       \
        U32 __s2 = (32 - (vb).bitlen) & 31; \
        if (__s2) {                         \
            VarBitsBPut((vb), 0, __s2);     \
        }                                   \
    }
#define VarBitsBPutFlush(vb) VarBitsBPutAlign(vb)
#define VarBitsBGetAlign(vb) \
    {                        \
        (vb).bitlen = 0;     \
    }
#define VarBitsBPutSize(vb)                               \
    ((U32)((((char*)(vb).cur) - ((char*)(vb).init)) * 8 + \
           (vb).bitlen))  // in bits !!
#define VarBitsBGetSize(vb)                               \
    ((U32)((((char*)(vb).cur) - ((char*)(vb).init)) * 8 - \
           (vb).bitlen))  // in bits !!
...
#define VarBitsBGet1(vb, temp_u32)                                       \
    (((vb).bitlen == 0)                                                  \
         ? (temp_u32 = *((BITSTYPE * RADRESTRICT)((vb).cur)),            \
            ...
            ((vb).bitlen) = (BITSTYPELEN - 1))                           \
         : (temp_u32 = ((vb).bits), ((vb).bits) <<= 1, --((vb).bitlen)), \

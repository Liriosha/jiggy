...
#define RR_PRAGMA_MESSAGE(str) \
    message(__FILE__ "(" RR_LINESTRING ") : message: " str)
...
#define RR_ASSERT_ALWAYS(exp)                                                  \
    do {                                                                       \
        if (!(exp)) {                                                          \
            if (rrDisplayAssertion(NULL, __FILE__, __LINE__, RR_FUNCTION_NAME, \
                                   #exp))                                      \
                RR_ASSERT_BREAK();                                             \
        }                                                                      \
    } while (0)
...
// evaluate to a constant result due to the fact that the underlying type is
// unsigned, such as comparisons of an unsigned int >= 0.  These need to be
// wrapped in macros so that they can be eliminated from compilers who will
// issue a warning or an error when you do such comparisons.
...
// if x already was unsigned, the compare against zero should get removed by the
// optimizer :
#define RR_CLAMP32_0_N(x, hi) \
    ((((U32)(x)) > (hi)) ? (((x) < 0) ? 0 : (hi)) : (x))
// this can be faster if x is actually signed (use signed shift to make mask)
// #define RR_CLAMP32_0_N(x,hi)  ( ( ((U32)(x)) > (hi) ) ? ( (~((x)>>31)) & (hi)
// )  : (x) )
...
#define RR_UNROLL_0(x) \
    do {               \
    } while (0)
#define RR_UNROLL_1(x) \
    do {               \
        x;             \
    } while (0)
#define RR_UNROLL_2(x) \
    do {               \
        x;             \
        x;             \
    } while (0)
#define RR_UNROLL_3(x) \
    do {               \
        x;             \
        x;             \
        x;             \
    } while (0)
#define RR_UNROLL_4(x) \
    do {               \
        x;             \
        x;             \
        x;             \
...
#define RR_UNROLL_5(x)  \
    do {                \
        RR_UNROLL_4(x); \
        x;              \
    } while (0)
#define RR_UNROLL_6(x)  \
    do {                \
        RR_UNROLL_5(x); \
        x;              \
    } while (0)
#define RR_UNROLL_7(x)  \
    do {                \
        RR_UNROLL_6(x); \
        x;              \
    } while (0)
#define RR_UNROLL_8(x)  \
    do {                \
        RR_UNROLL_4(x); \
        RR_UNROLL_4(x); \
    } while (0)
#define RR_UNROLL_9(x)  \
    do {                \
        RR_UNROLL_8(x); \
        x;              \
    } while (0)
#define RR_UNROLL_10(x) \
    do {                \
        RR_UNROLL_9(x); \
        x;              \
    } while (0)
#define RR_UNROLL_11(x)  \
    do {                 \
        RR_UNROLL_10(x); \
        x;               \
    } while (0)
#define RR_UNROLL_12(x)  \
    do {                 \
        RR_UNROLL_11(x); \
        x;               \
    } while (0)
#define RR_UNROLL_13(x)  \
    do {                 \
        RR_UNROLL_12(x); \
        x;               \
    } while (0)
#define RR_UNROLL_14(x)  \
    do {                 \
        RR_UNROLL_13(x); \
        x;               \
    } while (0)
#define RR_UNROLL_15(x)  \
    do {                 \
        RR_UNROLL_14(x); \
        x;               \
    } while (0)
#define RR_UNROLL_16(x) \
    do {                \
        RR_UNROLL_8(x); \
        RR_UNROLL_8(x); \
    } while (0)
...
#define RR_UNROLL_I_0(b, x) \
    do {                    \
    } while (0)
#define RR_UNROLL_I_1(b, x) \
    do {                    \
        const int i = b;    \
        x;                  \
    } while (0)
#define RR_UNROLL_I_2(b, x)      \
    do {                         \
        RR_UNROLL_I_1(b, x);     \
        RR_UNROLL_I_1(b + 1, x); \
    } while (0)
#define RR_UNROLL_I_3(b, x)      \
    do {                         \
        RR_UNROLL_I_2(b, x);     \
        RR_UNROLL_I_1(b + 2, x); \
    } while (0)
#define RR_UNROLL_I_4(b, x)      \
    do {                         \
        RR_UNROLL_I_2(b, x);     \
        RR_UNROLL_I_2(b + 2, x); \
    } while (0)
#define RR_UNROLL_I_5(b, x)      \
    do {                         \
        RR_UNROLL_I_4(b, x);     \
        RR_UNROLL_I_1(b + 4, x); \
    } while (0)
#define RR_UNROLL_I_6(b, x)      \
    do {                         \
        RR_UNROLL_I_4(b, x);     \
        RR_UNROLL_I_2(b + 4, x); \
    } while (0)
#define RR_UNROLL_I_7(b, x)      \
    do {                         \
        RR_UNROLL_I_4(b, x);     \
        RR_UNROLL_I_3(b + 4, x); \
    } while (0)
#define RR_UNROLL_I_8(b, x)      \
    do {                         \
        RR_UNROLL_I_4(b, x);     \
        RR_UNROLL_I_4(b + 4, x); \
    } while (0)
#define RR_UNROLL_I_9(b, x)      \
    do {                         \
        RR_UNROLL_I_8(b, x);     \
        RR_UNROLL_I_1(b + 8, x); \
    } while (0)
#define RR_UNROLL_I_10(b, x)     \
    do {                         \
        RR_UNROLL_I_8(b, x);     \
        RR_UNROLL_I_2(b + 8, x); \
    } while (0)
#define RR_UNROLL_I_11(b, x)     \
    do {                         \
        RR_UNROLL_I_8(b, x);     \
        RR_UNROLL_I_3(b + 8, x); \
    } while (0)
#define RR_UNROLL_I_12(b, x)     \
    do {                         \
        RR_UNROLL_I_8(b, x);     \
        RR_UNROLL_I_4(b + 8, x); \
    } while (0)
#define RR_UNROLL_I_13(b, x)     \
    do {                         \
        RR_UNROLL_I_8(b, x);     \
        RR_UNROLL_I_5(b + 8, x); \
    } while (0)
#define RR_UNROLL_I_14(b, x)     \
    do {                         \
        RR_UNROLL_I_8(b, x);     \
        RR_UNROLL_I_6(b + 8, x); \
    } while (0)
#define RR_UNROLL_I_15(b, x)     \
    do {                         \
        RR_UNROLL_I_8(b, x);     \
        RR_UNROLL_I_7(b + 8, x); \
    } while (0)
#define RR_UNROLL_I_16(b, x)     \
    do {                         \
        RR_UNROLL_I_8(b, x);     \
        RR_UNROLL_I_8(b + 8, x); \
    } while (0)
#define RR_UNROLL_I_17(b, x)      \
    do {                          \
        RR_UNROLL_I_16(b, x);     \
        RR_UNROLL_I_1(b + 16, x); \
    } while (0)
#define RR_UNROLL_I_24(b, x)      \
    do {                          \
        RR_UNROLL_I_16(b, x);     \
        RR_UNROLL_I_8(b + 16, x); \
    } while (0)
#define RR_UNROLL_I_32(b, x)       \
    do {                           \
        RR_UNROLL_I_16(b, x);      \
        RR_UNROLL_I_16(b + 16, x); \
    } while (0)

// RR_STRING_JOIN_DELAY is useful here if count is itself a macro
#define RR_UNROLL_N(count, x)                       \
    do {                                            \
        RR_STRING_JOIN_DELAY(RR_UNROLL_, count)(x); \
    } while (0)

#define RR_UNROLL_I_N(count, b, x)                       \
    do {                                                 \
        RR_STRING_JOIN_DELAY(RR_UNROLL_I_, count)(b, x); \
    } while (0)

//===========================================================================

// helper function to show I really am intending to put a pointer difference in
// an int :
static RADINLINE SINTa rrPtrDiff(SINTa val) { return val; }
static RADINLINE S32 rrPtrDiff32(SINTa val) {
    S32 ret = (S32)val;
    RR_ASSERT((SINTa)ret == val);
    return ret;
}
...
// This security warning could be useful, but we get it on some non-useful
// cases, like fopen()

...
RADDEFFUNC void ArithBitsPutBits(ARITHBITS* ab, U32 start, U32 range, U32 bits,
                                 U32 scale);
...
RADDEFFUNC void ArithBitsPutBitsValue(ARITHBITS* ab, U32 value, U32 bits,
                                      U32 scale);
...
#define ARITHBITS_PRECISION_BITS \
    9  // coding is done to this accuracy (in terms of range>>PRECISION_BITS)
...
#define ARITHBITS_EXTRA_BITS           \
    (((ARITHBITS_CODE_BITS - 1) % 8) + \
     1)  // == 7	== CODE_BITS - (8*(CODE_BYTES-1))

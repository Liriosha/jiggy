...
static RADINLINE rrbool rrIsPow2(const U32 x) { return !(x & ~(-(S32)x)); }

// rrNextPow2 : if x already is Pow2, returns x ; else returns the next higher
// power of 2 ; rrNextPow2 returns 1 for x <= 0
RADDEFFUNC U32 rrNextPow2(const U32 x);

// rrPrevPow2 : if x already is Pow2, returns x ; else returns the next lower
// power of 2 ; rrPrevPow2 returns 0 for x <= 0
...
static RADINLINE S32 rrNextPow2(const S32 x)
{
    ...
    rrassert( x >= 0 );
    // @@ could probably be faster, but not used anywhere important at the
moment while ( y < x )
    {
        y += y;
    }
...
static RADINLINE UINTa rrAlignUpA(const UINTa v, const S32 alignment) {
    return RR_STRING_JOIN(rrAlignUp, RAD_PTRBITS)(
        (RR_STRING_JOIN(U, RAD_PTRBITS))v, alignment);
}

// Aligns v down, alignment must be a power of two.
static RADINLINE UINTa rrAlignDownA(const UINTa v, const S32 alignment) {
    return RR_STRING_JOIN(rrAlignDown, RAD_PTRBITS)(
        (RR_STRING_JOIN(U, RAD_PTRBITS))v, alignment);
}
...
//  the only problem with that is if D3D or anyone changes the FPU settings
    ...
    // @@ URG - Edit & Continue fucks this up somehow, it blows up something in
    // the SSE state
...
static RADINLINE S32 rr_ftoi_round_positive(const F32 val) {
    // only correct for val >= 0 ; doesn't assert though, lets you pass
    // negatives, they are just wrong
...
//  getbitlevel(n) = ilog2ceil except on powers of two (on powers of two,
//  GetBitLevel is one larger)
//
// that is :
//	 (1<<getbitlevel(n))-1
